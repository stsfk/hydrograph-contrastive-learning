...
main_folder <- "/Volumes/Data/data/raw/CAMELS-DE/Version 1.0.0/timeseries/"
...
# save data
save(data_list, file = "./data/Q_raw_camels_de.Rda")
...
load("./data/Q_raw_camels_de.Rda")

data_process <- tibble(
  catchment_name =  names(data_list) %>% str_sub(31, -5),
  data = data_list
)
...
# Number of catchments retained: 1,203
...
save(data_processing_training,
     ...
     file = "./data/Q_filtered_camels_de.Rda")


# Prepare modeling data ---------------------------------------------------

load("./data/Q_filtered_camels_de.Rda")
...
write_csv(data_processing_training, file = 'data/training_data_camels_de.csv')
write_csv(data_processing_validation, file = 'data/validation_data_camels_de.csv')
write_csv(data_processing_test, file = 'data/test_data_camels_de.csv')


# Analyze data ------------------------------------------------------------
library(zoo)

training_tibble <- read_csv('data/training_data_camels_de.csv')
validation_tibble <- read_csv("data/validation_data_camels_de.csv")
test_tibble <- read_csv("data/test_data_camels_de.csv")
...
training_tibble$eligibility %>% sum() # 6,043,480possible 365-day hydrographs
validation_tibble$eligibility %>% sum() # 3,934,574 possible 365-day hydrographs
test_tibble$eligibility %>% sum() # 5,830,560 possible 365-day hydrographs
